...
- **胶囊两端的内容内缩**:`bar.padding` 之外再加圆角的 45° keyline 距离 `r·(1-1/√2)`(半径 15 时为 4px)。圆角端不是直边,按包围盒量出来的留白看起来小于实际值;图标网格用同一条规则让圆形和方形读起来一样大。此值与 radius 绑定,不可配置。
- **文本按 cap height 定位**:行盒包含 cap 之上的行距和 baseline 之下的降部空间,而 `09:19` 这类字符串一个像素都用不到,所以“把行盒居中/按行盒加内边距”会差 `(line_height - cap)/2`(本字体 11px 下为 3px)。栏内文字按 cap 居中,卡片文字让 cap 顶端距卡片上边正好一个 `card_padding`,CSS 的处理方法同名(`text-box: trim-both cap alphabetic`)。字体度量由首帧光栅化一个 `H` 量出并缓存(`TextEngine::cap_metrics`)。
