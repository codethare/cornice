...
- 代码注释与错误信息用英语,注释只解释非显然的决策(为什么),不解释代码在做什么。
- 所有命令在仓库根目录 `/home/goy0/bwrap/code_box/build/workspace/cornice` 下执行。
- 提交用 `jj commit -m "..."`(仓库是 jj/git 双存在),提交信息用英语。**提交前先确认 `cargo test` 通过。**
...
mod tests {
    ...
    fn blend_accumulates_alpha() {
        ...
        assert!((g as i32 - r as i32).abs() <= 1 && (b as i32 - r as i32).abs() <= 1, "三个通道应同为中灰");
    }
}
```
...
WLR_BACKENDS=headless WLR_RENDERER=pixman WLR_LIBINPUT_NO_DEVICES=1 sway -c /dev/null >/tmp/sway.log 2>&1 &
sleep 3
mkdir -p /tmp/cornice-testhome/cornice
cat > /tmp/cornice-testhome/cornice/config.toml <<'EOF'
[bar.right]
modules = [ { kind = "exec", command = "while true; do date +%s; sleep 1; done", format = "t={out}" } ]
EOF
XDG_CONFIG_HOME=/tmp/cornice-testhome WAYLAND_DISPLAY=wayland-1 timeout 4 ./target/debug/cornice; echo "exit=$?"
```
